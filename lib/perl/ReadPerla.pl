...
GetOptions('target=s'=>\$target,'assembly=s'=>\$assembly,'mode=s'=>\$mode,'mapq=i'=>\$MAPQ,'verbose|v'=>\$verbose,'help|h'=>\$help,'man|m'=>\$man,) or pod2usage ();
...
while($record=<CHECKBOOK>){
  ...
  system qq($Program_Folder_Path/lib/bash/./BAM2RC.sh $Bam_File_Path $MAPQ $Sample_Out_Folder_Path $Program_Folder_Path $Sample_Label $target $assembly);
  print "Removing temporary files...\n";
  rmtree("$Sample_Out_Folder_Path/.tmp");
  print "Read Count done!\n";
  print "Normalizing Read Count Data...\n";
  system qq(R --slave --args $Program_Folder_Path,$Sample_Out_Folder_Path,$Sample_Label,$target,$assembly < $Program_Folder_Path/lib/R/XCAVATORNormalizationGenome.R);
  print "Normalization done!\n";
  }
  if ($mode eq "DOC") {
  print "Creating Depth of Coverage Data...\n";
  system qq($Program_Folder_Path/lib/bash/./BAM2DOC.sh $Bam_File_Path $MAPQ $Sample_Out_Folder_Path $Program_Folder_Path $Sample_Label $target $assembly);
  print "Removing temporary files...\n";
  rmtree("$Sample_Out_Folder_Path/.tmp");
  print "Read Count done!\n";
  print "Normalizing Depth of Coverage Data...\n";
  system qq(R --slave --args $Program_Folder_Path,$Sample_Out_Folder_Path,$Sample_Label,$target,$assembly < $Program_Folder_Path/lib/R/XCAVATORNormalizationGenome.R);
  print "Normalization done!\n";
  }
